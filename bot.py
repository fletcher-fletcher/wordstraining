...
bot.timeout = 30  # Увеличиваем таймаут
...
def format_word_card(word, show_stats=True, user_id=None):
    ...
    # Добавляем статистику, если есть user_id
...
def send_random_word(chat_id, user_id):
    """Отправляет случайное слово"""
    word = get_random_word()
    if word:
        card = format_word_card(word, user_id=user_id)
        
        markup = telebot.types.InlineKeyboardMarkup(row_width=2)
        save_btn = telebot.types.InlineKeyboardButton(
            "📥 Сохранить", 
            callback_data=f"save_{word['id']}"
        )
        voice_btn = telebot.types.InlineKeyboardButton(
            "🔊 Произнести", 
            callback_data=f"voice_{word['id']}"
        )
...
def mylist_command(message):
    ...
    # Показываем по 10 слов с пагинацией
...
def show_words_page(chat_id, user_id, words, page=0, edit_message_id=None):
    """Показывает страницу со словами"""
    page_size = 10
    ...
    # Кнопка тренировки
    practice_btn = telebot.types.InlineKeyboardButton(
        "🎯 Тренироваться по своим словам", 
        callback_data="practice_mode_mylist"
    )
...
def practice_choice(message):
    ...
    btn_all = telebot.types.InlineKeyboardButton(
        "🌍 По всем словам", 
        callback_data="practice_mode_all"
    )
    
    if saved_count > 0:
        btn_mylist = telebot.types.InlineKeyboardButton(
            f"📚 По моим словам ({saved_count})", 
            callback_data="practice_mode_mylist"
        )
...
def start_practice_session(user_id, mode, chat_id):
    ...
    for opt in options:
        btn = telebot.types.InlineKeyboardButton(
            f"🔸 {opt}", 
            callback_data=f"practice_answer_{word['id']}_{opt == word['translation']}"
        )
        markup.add(btn)
    
    # Кнопки управления
    show_btn = telebot.types.InlineKeyboardButton(
        "👀 Показать ответ", 
        callback_data=f"practice_show_{word['id']}"
    )
    voice_btn = telebot.types.InlineKeyboardButton(
        "🔊 Послушать", 
        callback_data=f"voice_{word['id']}"
...
def handle_callback(call):
    ...
    if call.data == "menu_practice":
        bot.delete_message(chat_id, message_id)
        # Создаем фейковое сообщение для practice_choice
        class FakeMessage:
    ...
    if call.data == "practice_mode_all":
        bot.edit_message_text(
            "🎯 Начинаем тренировку по *всем словам*!",
            chat_id,
            message_id,
            parse_mode='Markdown'
        )
    ...
    if call.data == "practice_mode_mylist":
        bot.edit_message_text(
            "🎯 Начинаем тренировку по *твоим словам*!",
            chat_id,
            message_id,
            parse_mode='Markdown'
        )
    ...
    if call.data.startswith("practice_answer_"):
        ...
        if word_data:
            ...
            save_btn = telebot.types.InlineKeyboardButton(
                "📥 Сохранить", 
                callback_data=f"save_{word_id}"
            )
            voice_btn = telebot.types.InlineKeyboardButton(
                "🔊 Послушать", 
                callback_data=f"voice_{word_id}"
            )
            continue_btn = telebot.types.InlineKeyboardButton(
                "🎯 Далее", 
    ...
    try:
        ...
        while True:
            try:
                ...
                logger.error(f"Ошибка подключения: {e}. Переподключение через 5 секунд...")
                time.sleep(5)
                continue
            except Exception as e:
